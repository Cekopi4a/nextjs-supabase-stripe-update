    ...
    <div className="flex flex-col h-full">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex justify-start">
              <p className="text-xs text-muted-foreground">
                {userRole === "trainer" ? "Треньор" : userRole === "client" ? "Клиент" : userRole}
              </p>
            </div>
          </div>
        ) : (
      ...
      <nav className={cn(
        ...
        {menuItems.map((item, index) => {
          const { label, href, icon: Icon, disabled = false, badge, isPro = false, isSection = false } = item;
          ...
          if (isSection && (!isCollapsed || isMobile)) {
            return (
              <div key={index} className={cn(
                "pt-5 pb-2 px-3 mt-3",
                index !== 0 && "border-t-2 border-primary/20"
              )}>
                <div className="flex items-center gap-2.5 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-lg px-3 py-2">
          ...
          // Skip section header in collapsed mode
          if (isSection && isCollapsed && !isMobile) {
            return null;
          }
